...
class FeatureClustering(nn.Module):
    ...
    def __init__(self, feature_dimension: int, num_artifact_clusters: int, calibration_hidden_layer_sizes: List[int]):
        ...
        self.num_nonartifact_clusters = 2

        # the 0th cluster is non-artifact
        self.num_clusters = self.num_artifact_clusters + self.num_nonartifact_clusters
        ...
        # this is a single parameter for holding all pre-softmax weights, but the nonartifact weights and
        # artifact weights are sub-tensors that are softmaxed separately
        self.cluster_weights_pre_softmax_vk = Parameter(torch.ones(len(Variation), self.num_clusters))
    ...
    def weighted_log_likelihoods_bk(self, ref_bre: RaggedSets, alt_bre: RaggedSets, ref_counts_b: IntTensor, alt_counts_b: IntTensor, var_types_b: IntTensor):
        ...
        nonartifact_weights_presoftmax_vk = self.cluster_weights_pre_softmax_vk[:, :self.num_nonartifact_clusters]
        artifact_weights_presoftmax_vk = self.cluster_weights_pre_softmax_vk[:, self.num_nonartifact_clusters:]

        log_nonartifact_cluster_weights_vk = torch.log_softmax(nonartifact_weights_presoftmax_vk, dim=-1)
        log_artifact_cluster_weights_vk = torch.log_softmax(artifact_weights_presoftmax_vk, dim=-1)

        log_nonartifact_cluster_weights_bk = log_nonartifact_cluster_weights_vk[var_types_b]
        log_artifact_cluster_weights_bk = log_artifact_cluster_weights_vk[var_types_b]

        log_lks_bk[:, :self.num_nonartifact_clusters] += log_nonartifact_cluster_weights_bk
        log_lks_bk[:, self.num_nonartifact_clusters:] += log_artifact_cluster_weights_bk
        return log_lks_bk
