...
class FeatureClustering(nn.Module):
    ...
    def __init__(self, feature_dimension: int, num_artifact_clusters: int, calibration_hidden_layer_sizes: List[int]):
        ...
        # very easy.
        self.alt_stdev_pre_exp_vk = Parameter(torch.zeros(len(Variation), self.num_clusters))
        self.ref_stdev_pre_exp_vk = Parameter(torch.zeros(len(Variation), self.num_clusters))

        self.cluster_weights_pre_softmax_vk = Parameter(torch.ones(len(Variation), self.num_artifact_clusters))

    def log_likelihoods(self, reads_bre: RaggedSets, centroids_vke, stdev_pre_exp_vk, counts_b: IntTensor, var_types_b: IntTensor):
        ...
        stdev_bk = torch.exp(stdev_pre_exp_vk)[var_types_b]
        stdev_rk = torch.repeat_interleave(stdev_bk, repeats=counts_b, dim=0)
        log_lks_rk = -(self.feature_dim / 2) * torch.log(stdev_rk) - torch.square(dist_rk) / (2 * torch.square(stdev_rk))
    ...
    def calculate_logits(self, ref_bre: RaggedSets, alt_bre: RaggedSets, ref_counts_b: IntTensor, alt_counts_b: IntTensor, var_types_b: IntTensor):
        ...
        alt_log_lks_bk = self.log_likelihoods(reads_bre=alt_bre, centroids_vke=self.alt_centroids_vke,
            stdev_pre_exp_vk=self.alt_stdev_pre_exp_vk, counts_b=alt_counts_b, var_types_b=var_types_b)

        ref_log_lks_bk = self.log_likelihoods(reads_bre=ref_bre, centroids_vke=self.ref_centroids_vke,
                                              stdev_pre_exp_vk=self.ref_stdev_pre_exp_vk, counts_b=ref_counts_b,
