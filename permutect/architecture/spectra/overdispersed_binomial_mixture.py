...
from permutect.utils.stats_utils import gamma_binomial_log_lk
...
class OverdispersedBinomialMixture(nn.Module):
    ...
    def moments_of_underlying_beta_mixture(self, var_type: int):
        ...
        component_log_means = torch.digamma(alphas) - torch.digamma(
            alphas + betas
        )  # digamma broadcasts to make 1D tensor
    ...
    def sample(self, types_b, n):
        ...
        dist = (
            torch.distributions.beta.Beta(alphas, betas)
            if self.mode == "beta"
            else torch.distributions.gamma.Gamma(alphas, betas)
        )
    ...
    def spectrum_density_vs_fraction(self, variant_type: Variation, depth: int):
        ...
        if self.mode == "none":
            ...
            dist = (
                torch.distributions.beta.Beta(alphas_k, betas_k)
                if self.mode == "beta"
