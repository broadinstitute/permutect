...
def calculate_pruning_thresholds(labeled_only_pruning_loader, model: ArtifactModel, label_art_frac: float, training_params: TrainingParameters) -> tuple[float, float]:
    ...
    if confident_art_total == 0 or confident_nonart_total == 0:
        raise ValueError("No data passed confidence thresholds; cannot estimate error rates. "
    ...
    if error_rate_sum >= 1.0:
        raise ValueError(f"Sum of error rates ({error_rate_sum:.3f}) >= 1.0; "
                         "rank pruning model assumptions are violated.")
    inv_art_error_rate = (nonart_error_rate / label_art_frac) * (label_nonart_frac - art_error_rate) / (1 - error_rate_sum)
    inv_nonart_error_rate = (art_error_rate / label_nonart_frac) * (label_art_frac - nonart_error_rate) / (1 - error_rate_sum)
...
def generated_pruned_data_for_fold(art_threshold: float, nonart_threshold: float, pruning_base_data_loader, model: ArtifactModel) -> Generator[Datum, None, None]:
...
def generate_pruned_data_for_all_folds(fold_datasets: List[ReadsDataset], model: ArtifactModel, training_params: TrainingParameters, tensorboard_dir) -> Generator[Datum, None, None]:
    ...
    for pruning_fold, fold_dataset in enumerate(fold_datasets):
        ...
        pruning_base_data_loader = fold_dataset.make_data_loader(training_params.batch_size, use_gpu, training_params.num_workers)
