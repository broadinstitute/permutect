...
class Checkpoint:
    ...
    def save_checkpoint_if_needed(self, epoch, loss: torch.Tensor):
        ...
        if self.best_checkpoint is None or (
            loss.item() < self.best_checkpoint["loss"] and not loss.isnan().item()
        ):
            print(f"New best mean loss: {loss.item():.1f}, saving new checkpoint.")
            save_data = {
                constants.STATE_DICT_NAME: self.model.state_dict(),
    ...
    def load_checkpoint_if_needed(self, loss: torch.Tensor):
        if self.best_checkpoint is not None and (
            loss.isnan().item() or loss.item() > 2 * self.best_checkpoint["loss"]
        ):
