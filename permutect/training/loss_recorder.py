...
class LossRecorder:
    ...
    def record(self, output: BatchOutput, losses: BatchLosses, batch: Batch):
        ...
        self.semisupervised_loss_metrics.record(
            batch, losses.supervised_losses_b, is_labeled_b * output.weights
        )
        self.semisupervised_loss_metrics.record(
            batch, losses.unsupervised_losses_b, (1 - is_labeled_b) * output.weights
        )
        self.source_prediction_loss_metrics.record(
            batch, losses.source_prediction_losses_b, output.source_weights
        )
    ...
    def output_results(self, epoch_type, epoch, summary_writer, generate_plots):
        ...
        self.semisupervised_loss_metrics.write_to_summary_writer(
            epoch_type, epoch, summary_writer, prefix="semisupervised-loss"
        )
        self.alt_count_loss_metrics.write_to_summary_writer(
            epoch_type, epoch, summary_writer, prefix="alt-count-loss"
        )
        self.source_prediction_loss_metrics.write_to_summary_writer(
            epoch_type, epoch, summary_writer, prefix="source-loss"
        )
        self.semisupervised_loss_metrics.report_marginals(
            f"Semisupervised loss for {epoch_type.name} epoch {epoch}."
        )
        if self.num_sources > 1:
            self.source_prediction_loss_metrics.report_marginals(
                f"Source prediction loss for {epoch_type.name} epoch {epoch}."
            )

        if generate_plots:
            self.semisupervised_loss_metrics.make_plots(
                summary_writer, "semisupervised loss", epoch_type, epoch
            )
            self.semisupervised_loss_metrics.make_plots(
                summary_writer,
            ...
            self.alt_count_loss_metrics.make_plots(
                summary_writer, "alt count prediction loss", epoch_type, epoch
            )
            if self.num_sources > 1:
                self.source_prediction_loss_metrics.make_plots(
