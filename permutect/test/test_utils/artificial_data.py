...
def make_random_data(
    art_gatk_info_gen: RandomGATKInfoGenerator,
    var_gatk_info_gen: RandomGATKInfoGenerator,
    art_read_gen: RandomReadGenerator,
    var_read_gen: RandomReadGenerator,
    num_data: int,
    ...
    for _ in range(0, num_data):
        ...
        alt_count = (
            random.randint(3, 10)
            if (artifact or (is_training_data and downsample_variants_to_match_artifacts))
...
def make_random_strand_bias_data(
    num_data: int,
    artifact_fraction=0.5,
    unlabeled_fraction=0.1,
    ref_downsampling=10,
...
def make_two_gaussian_data(
    ...
    return make_random_data(
        art_gatk_info_gen=art_gatk_info_gen,
        var_gatk_info_gen=var_gatk_info_gen,
        art_read_gen=art_read_gen,
        var_read_gen=var_read_gen,
        num_data=num_data,
        ...
        ref_downsampling=ref_downsampling,
...
def make_wide_and_narrow_gaussian_data(
    num_data,
    is_training_data=True,
    vaf=0.5,
    artifact_fraction=0.5,
