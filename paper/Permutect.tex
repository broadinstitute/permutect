...
Compared to other leading somatic variant calling tools, Permutect has a better balance of sensitivity and precision ($F_1$ score), more consistent performance between tumor-only and tumor-normal settings, low computational requirements, and requires extremely little data for training new models.
\end {abstract}
%TC:break main
%the command above serves to have a word count for the abstract
...
Thanks to these two insights Permutect easily adapts to different sequencing technologies, works for both somatic and germline calling, can be trained on a very modest amount of data, and needs only germline data for training, regardless of the type of calling it needs to do.
...
% Put this where you want the figure
\begin{figure}[htbp]
  \dummyfig{Placeholder: Synthetic BAM results here}
  \caption{Several sub-figures for allele fractions of 5\%, 10\%, 25\%, and 50\% synthetic (BAMSurgeon) tumor-only and tumor-normal calls.  Separate plots for SNV/indel, WES/WGS.  x-axis is tool, y-axis is sensitivity, precision, $F_1$.}
  \label{fig:placeholder}
\end{figure}

% Put this where you want the figure
\begin{figure}[htbp]
  \dummyfig{Placeholder: DREAM challenge results here}
  \caption{Several sub-figures for DREAM challenges 1, 2, 3, and 4 tumor-only and tumor-normal calls.  Separate plots for SNV/indel, WES/WGS.  x-axis is tool, y-axis is sensitivity, precision, $F_1$.}
  \label{fig:placeholder}
\end{figure}
...
Additionally, the SEQC2 cell line was prepared in FFPE conditions with different digestion times and sequenced.

% Put this where you want the figure
\begin{figure}[htbp]
  \dummyfig{Placeholder: SEQC2 results here}
  \caption{Sensitivity, precision, and $F_1$ scores on the SEQC2 HiSeqX and Novaseq replicates.  Separate plots for SNV/indel, WES/WGS.  x-axis is tool, y-axis is sensitivity, precision, $F_1$.}
  \label{fig:placeholder}
\end{figure}

% Put this where you want the figure
\begin{figure}[htbp]
  \dummyfig{Placeholder: SEQC2 FFPE results here}
  \caption{Sensitivity, precision, and $F_1$ scores on the SEQC2 FFPE digestion time series.  Separate plots for SNV/indel, WES/WGS. x-axis is tool (with multiple bars for tool for digestion times of 2, 4, 6, and 24 hours), y-axis is sensitivity, precision, $F_1$.}
  \label{fig:placeholder}
\end{figure}

% Put this where you want the figure
\begin{figure}[htbp]
  \dummyfig{Placeholder: SEQC2 depth and allele fraction series}
  \caption{Sensitivity, precision, and $F_1$ scores on the SEQC2 WGS depth series.  Separate plots for SNV/indel and different tumor:normal ratios (1:1, 1:3, 1:9, 1:19). x-axis is tool (with multiple bars for tool for depth of 30x, 50x, 100x, and 300x), y-axis is sensitivity, precision, $F_1$.}
  \label{fig:placeholder}
\end{figure}

\subsection*{Computational Resources}
We compared Permutect's RAM requirements and runtime against other tools.

% Put this where you want the figure
\begin{figure}[htbp]
  \dummyfig{Placeholder: RAM and CPU}
  \caption{RAM requirements and CPU/GPU time for different tools' pipeline on WGS samples of different depths.  Two plots -- one RAM, one runtime -- each with multiple lines -- one line per tool.  x-axis is average depth 30x, 50x, 100x, 300x; y-axis is RAM in Gb or runtime in hours.}
  \label{fig:placeholder}
\end{figure}

\section*{Methods}
\subsection*{Labeling training data} 
One creates a Permutect dataset by running Mutect2\footnote{This is one good reason not to name it Mutect3.} in a special Permutect dataset mode.  If a germline truth VCF is available, any variant contained in that VCF is considered to be a non-artifact example\footnote{Recall that the Permutect neural network is responsible for classifying artifacts vs non-artifacts, not somatic vs germline variants.} while any Mutect2 call missing from that VCF is considered an artifact.  Otherwise, any variant that is common in the population and is supported by a sufficient fraction of reads is considered a non-artifact\footnote{Because germline variants are plentiful these thresholds can be fairly strict without sacrificing too much training data.} and anything called by Mutect2 (without running any subsequent artifact filtering such as FilterMutectCalls) that is supported by a small fraction of reads and is rare in the population is considered an artifact.  Everything else is considered unlabeled data and is used in semi-supervised learning.
...
Next, data must be converted into tensors for computation.  We designed this with simplicity in mind and we expect fancier approaches to be fruitful in the future.  A single input\footnote{We ignore batching here for simplicity but in practice, a batch of candidate variants is the actual unit of processing.} to the artifact model consists of i) a set of reads supporting the reference allele (``ref"), and ii) a set of reads supporting the non-reference allele (``alt"), with each read encoded as a vector; iii) 21 bases of the reference and alt haplotypes, centered at the variant start position\footnote{21 bases is large enough to encompass almost any sequence motifs, such as homopolymers, that would predispose the sequencing machine to errors, and small enough for computational efficiency.  Centering the window at the variant start allows the model to learn that this is a privileged position.  A homopolymer where the variant begins is different from one ten bases away.}; and iv) a vector of additional features pertaining to the variant.  We will now describe each of these in detail,
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.5\textwidth]{Figures/artifact.png}
    \caption{Artifact model overall view.  NOTE: this figure comes from an AI tool to turn hand-drawn sketches into diagrams.  IT IS NOT FINAL!!!}
    \label{fig:artifact}
\end{figure}

\subsection*{Artifact Model Overview} 
The artifact model (see Figure \ref{fig:artifact}) starts with the sets of ref and alt read vectors, the ref and alt haplotypes, and the variant feature vector.  It first processes all vectors independently, applying an MLP to all reads, a distinct MLP to the variant feature vector, and a one-dimensional convolutional network to the haplotypes.  Then the resulting transformed variant feature vectors and haplotype vectors are concatenated with each transformed read vector.  At this point, every read vector has been processed in isolation.  Next, the processed ref and alt reads are sent through several symmetric gated MLP blocks, which outputs new sets of ref and alt reads, each influenced by attention to other reads.  The symmetric gated MLP blocks are both permutation-invariant and count-agnostic.  That is, they are invariant to the order of ref and alt reads and each read only pays attention to the average of other reads, which erases knowledge of the total count of ref and alt reads.  After the gated MLP blocks, there are still sets of reads that need to be condensed into a single vector.  This is accomplished with a symmetric set pooling network, which is also permutation-invariant and count-agnostic.  Now the input sets are summarized into a single vector representation of the candidate variant, to which Permutect applies a linear transformation to obtain a single logit expressing the odds that the variant is an artifact.
...
At this point we have transformed sets of ref and alt reads in a permutation-invariant manner, but we still need to reduce the set of transformed reads into a scalar.  Permutect achieves this with a generative likelihood model that assumes reads are conditionally i.i.d. given a discrete variable indicating different varieties of artifacts.  Each potential variant has a one-hot indicator variable $\mathbf{z}$ such that $z_0 = 1$ for non-artifacts and $z_k = 1$ for artifacts of type $k = 1, 2 \ldots K$.  If $z_0 = 1$, alt reads are assumed to follow an origin-centered normal distribution:
\begin{equation}
\mathbf{a} \sim N(\mathbf{0}, \mathbf{\Sigma}), \label{eq:nonart}
\end{equation}
where $\mathbf{\Sigma}$ is a learned parameter.  Each artifact type $k$ is associated with a unit vector $\hat{\mathbf{v}}_k$ along which reads deviate from the origin.  Reads' projections along this direction are assumed to follow exponentially-modified Gaussian (EMG) distributions, while their components orthogonal to this direction are assumed to follow isotropic Gaussian distributions.  That is, if $z_k = 1$ for $k > 0$ we have:
\begin{align}
\mathbf{a} \cdot \hat{\mathbf{v}}_k &\sim \mathrm{EMG}(\mu_k, \sigma_k, \lambda_k) \label{eq:art_parallel} \\
|\mathbf{a} - \mathbf{a} \cdot \hat{\mathbf{v}}_k| &\sim N(0, \tau_k) \label{eq:art_orthogonal}
\end{align} 
According to conditional independence, we sum the log-likelihoods of all alt reads given by Equations \ref{eq:nonart}, \ref{eq:art_parallel}, and \ref{eq:art_orthogonal} to obtain the likelihoods for the indicator $\mathbf{z}$, hence the likelihood and the binary logit for the call of artifact versus non-artifact.  In addition to conveniently and efficiently reducing the set of reads to a single binary logit for the variant as a whole, this generative approach automatically encodes a correct probabilistic treatment of different quantities of evidence and numbers of supporting reads.  Permutect is more confident of predictions based on more reads.  This is worth stressing: a trained Permutect model is not only accurate, its predictions are well-calibrated and its degree of confidence can be trusted.
